...
export default function SigninForm() {
  ...
  const handleRoleRedirect = (role: string | undefined): void => {
    switch (role) {
      case "admin":
  ...
  const onSubmit: SubmitHandler<SignInFormData> = async (values) => {
    ...
    const doSignin = async (): Promise<LoginResponse> => {
      const payload = {
        email: values.email,
  ...
  const fetchAvatarByEmail = async (email: string): Promise<void> => {
    ...
    try {
      ...
      const user: UserData = data?.user || (Array.isArray(data) ? data[0] : data);
      if (user) {
        setAvatarPreview(
          user.avatarUrl || user.profilePicture || user.avatar || user.photo || null
        );
      } else {
          ...
          <div className="w-20 h-20 rounded-full overflow-hidden bg-teal-50 flex items-center justify-center mb-2 border-2 border-teal-100">
            {avatarPreview ? (
              ...
              <svg
                ...
                fill="currentColor"
              >
                <path d="M12 12a5 5 0 100-10 5 5 0 000 10zm0 2c-6.075 0-11 3.582-11 8v2h22v-2c0-4.418-4.925-8-11-8z" />
              </svg>
            )}
          </div>
          <h2
            ...
            <FormItem>
              ...
              <FormControl>
                <Input
                  ...
                  onBlur={(e) => {
                    field.onBlur();
                    fetchAvatarByEmail(e.currentTarget.value);
                  }}
                  onChange={(e) => {
                    field.onChange(e);
